...
func TestAAJWTCLI(t *testing.T) {
	...
	// load the test private key from jwk module
	...
	createAudienceClaimHash, err := testlib.ExecTx(t, ctx, xion.GetNode(),
		xionUser.KeyName(),
	...
	txDetails, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "tx", createAudienceClaimHash)
	require.NoError(t, err)
	t.Logf("TxDetails: %s", txDetails)

	createAudienceHash, err := testlib.ExecTx(t, ctx, xion.GetNode(),
		xionUser.KeyName(),
		"jwk", "create-audience",
		aud,
...
func TestAABasic(t *testing.T) {
	...
	account, err := testlib.ExecBin(t, ctx, xion.GetNode(),
		"keys", "show",
		xionUser.KeyName(),
	...
	contractState, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contract-state", "smart", aaContractAddr, `{"authenticator_by_i_d":{ "id": 0 }}`)
	require.NoError(t, err)
	...
	jsonMsg := testlib.RawJSONMsgSend(t, aaContractAddr, recipientKeyAddress, xion.Config().Denom)
	require.NoError(t, err)
	...
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), rotateFile)
	require.NoError(t, err)
	...
	jsonExecMsg = testlib.RawJSONMsgExecContractRemoveAuthenticator(aaContractAddr, aaContractAddr, 0)
	require.True(t, json.Valid(jsonExecMsg))
	...
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), removeFile)
	require.NoError(t, err)
...
func TestAAClientEvent(t *testing.T) {
	ctx := t.Context()
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	...
	// Store Wasm Contract
	codeID, err := xion.StoreContract(ctx, xionUser.FormattedAddress(),
		testlib.IntegrationTestPath("testdata", "contracts", "account-wasm-updatable-event-aarch64.wasm"))
	require.NoError(t, err)
