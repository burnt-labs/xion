...
func TestAppMintNoInflationNoFees(t *testing.T) {
	ctx := t.Context()
	if testing.Short() {
	...
	// Run test harness
	testlib.VerifyMintModuleTestRandomBlocks(t, xion, ctx, assertion)
}
...
func TestAppMintInflationNoFees(t *testing.T) {
	ctx := t.Context()
...
func sendPeriodicBankTx(t *testing.T, chain *cosmos.CosmosChain, ctx context.Context, chainHeight uint64, duration int, txHashes *MintModuleTest, feeTypeHigh bool) {
	...
	for curHeight < chainHeight {
		...
		blockProvisionDec := testlib.GetBlockAnnualProvision(t, chain, ctx, chain.Config().Denom, curHeight) // This the minted token amount for the current block
...
func TestAppMintInflationHighFees(t *testing.T) {
	ctx := t.Context()
	...
	// Run test harness
	testlib.VerifyMintModuleTest(t, xion, ctx, txHashes.TxHashes, assertion)
}

// Here we test the mint module by sending a bunch of transactions with extra low fees
// and checking if the total supply increases. We also check if the total supply
// increases by the correct amount.
func TestAppMintInflationLowFees(t *testing.T) {
	ctx := t.Context()
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
