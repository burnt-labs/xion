...
func TestAppTokenFactory(t *testing.T) {
	...
	tfDenom := testlib.CreateTokenFactoryDenom(t, ctx, xion, xionUser, "ictestdenom", fmt.Sprintf("0%s", xion.Config().Denom))
	t.Log("tfDenom", tfDenom)

	// mint
	testlib.MintTokenFactoryDenom(t, ctx, xion, xionUser, 100, tfDenom)
	...
	testlib.MintToTokenFactoryDenom(t, ctx, xion, xionUser, xionUser2, expectedMint, tfDenom)
	t.Log("minted tfDenom to user")
	...
	codeID, err := xion.StoreContract(ctx, xionUser.FormattedAddress(),
		testlib.IntegrationTestPath("testdata", "contracts", "tokenfactory_core.wasm"))
	require.NoError(t, err)
	...
	testlib.TransferTokenFactoryAdmin(t, ctx, xion, xionUser, coreTFContract, tfDenom)

	// ensure the admin is the contract
	admin := testlib.GetTokenFactoryAdmin(t, ctx, xion, tfDenom)
	t.Log("admin", admin)
