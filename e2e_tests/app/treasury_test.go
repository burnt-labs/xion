...
func TestAppTreasuryContract(t *testing.T) {
	ctx := t.Context()
	if testing.Short() {
	...
	err = testutil.WaitForBlocks(ctx, 2, xion)
	require.NoError(t, err)
	contractState, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contract-state", "all", treasuryAddr)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), sendFile)
	require.NoError(t, err)

	sendFilePath := strings.Split(sendFile.Name(), "/")

	signedTx, err := testlib.ExecBinRaw(t, ctx, xion.GetNode(),
	...
	res, err := testlib.ExecBroadcastWithFlags(t, ctx, xion.GetNode(), signedTx, "--output", "json")
	require.NoError(t, err)
	t.Logf("broadcasted tx: %s", res)

	txDetails, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "tx", res)
	require.NoError(t, err)
	...
	feeGrantDetails, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "feegrant", "grants-by-grantee", granteeUser.FormattedAddress())
	require.NoError(t, err)
	...
	require.NoError(t, err)
	bankSend, err := testlib.ExecTx(t, ctx, xion.GetNode(), granteeUser.KeyName(), []string{
	...
	txDetails, err = testlib.ExecQuery(t, ctx, xion.GetNode(), "tx", bankSend)
	require.NoError(t, err)
	...
	err = testutil.WaitForBlocks(ctx, 2, xion)
	require.NoError(t, err)
	userMapContractState, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contract-state", "all", userMapAddr)
	require.NoError(t, err)
	...
	userMapUpdateHash, err := testlib.ExecTx(t, ctx, xion.GetNode(), granteeUser.KeyName(), []string{
	...
	txDetails, err = testlib.ExecQuery(t, ctx, xion.GetNode(), "tx", userMapUpdateHash)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), revokeSendFile)
	require.NoError(t, err)

	revokeSendFilePath := strings.Split(revokeSendFile.Name(), "/")

	revokeSignedTx, err := testlib.ExecBinRaw(t, ctx, xion.GetNode(),
	...
	res, err = testlib.ExecBroadcastWithFlags(t, ctx, xion.GetNode(), revokeSignedTx, "--output", "json")
	require.NoError(t, err)
	t.Logf("broadcasted tx: %s", res)
...
func TestAppTreasuryMulti(t *testing.T) {
	ctx := t.Context()
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
