...
// Global variables to hold the file location arguments
// Primary path uses e2e_tests (current branch structure)
// Fallback path uses integration_tests (main branch structure)
var (
	configFileUrl         = "https://raw.githubusercontent.com/burnt-labs/xion/refs/heads/main/e2e_tests/testdata/unsigned_msgs/plain_config.json"
	configFileUrlFallback = "https://raw.githubusercontent.com/burnt-labs/xion/refs/heads/main/integration_tests/testdata/unsigned_msgs/plain_config.json"
)

func TestAppUpdateTreasuryConfigs(t *testing.T) {
	ctx := t.Context()
	...
	configData, err := os.ReadFile(testlib.IntegrationTestPath("testdata", "unsigned_msgs", "plain_config.json"))
	require.NoError(t, err)
	...
	// URL Test with fallback support
	...
	// If primary URL fails with 404, try fallback URL (main branch uses integration_tests instead of e2e_tests)
	if err != nil && strings.Contains(err.Error(), "404") {
		t.Logf("Primary URL failed with 404, trying fallback URL: %s", configFileUrlFallback)
		cmd[3] = configFileUrlFallback // Replace URL argument
		_, err = testlib.ExecTx(t, ctx, xion.GetNode(), xionUser.KeyName(), cmd...)
	}
	require.NoError(t, err)
...
func validateGrantConfigs(t *testing.T, ctx context.Context, xion *cosmos.CosmosChain, treasuryAddr string, expected int, msgs ...string) {
	...
	grantQueryRaw, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contract-state", "smart", treasuryAddr, string(grantQueryMsgStr))
	require.NoError(t, err)
...
func validateFeeConfig(t *testing.T, ctx context.Context, xion *cosmos.CosmosChain, treasuryAddr string) {
	...
	feeQueryRaw, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contract-state", "smart", treasuryAddr, string(feeQueryMsgStr))
	require.NoError(t, err)
...
func TestAppUpdateTreasuryConfigsAA(t *testing.T) {
	ctx := t.Context()
	...
	_, err = testlib.ExecBin(t, ctx, xion.GetNode(),
		"keys", "show",
		xionUser.KeyName(),
	...
	// Store AA Wasm Contract from abstract-account module
	...
	contractsResponse, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "wasm", "contracts", codeID)
	require.NoError(t, err)
	aaContractAddr := contractsResponse["contracts"].([]interface{})[0].(string)
	t.Logf("AA Contract Address: %s", aaContractAddr)

	treasuryCodeID, err := xion.StoreContract(ctx, xionUser.FormattedAddress(),
		testlib.IntegrationTestPath("testdata", "contracts", "treasury-aarch64.wasm"))
	require.NoError(t, err)
	...
	treasuryAddr1, err := xion.InstantiateContract(ctx, xionUser.KeyName(), treasuryCodeID, string(instantiateMsgStr), true)
	require.NoError(t, err)
	treasuryAddr2, err := xion.InstantiateContract(ctx, xionUser.KeyName(), treasuryCodeID, string(instantiateMsgStr), true)
	require.NoError(t, err)
	...
	unsignedTx, err := testlib.ExecBin(t, ctx, xion.GetNode(), cmd...)
	require.NoError(t, err)
	...
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), unsignedTxFile)
	require.NoError(t, err)

	unsignedTxFilePath := strings.Split(unsignedTxFile.Name(), "/")

	_, err = testlib.ExecBinRaw(t, ctx, xion.GetNode(),
	...
	if err != nil && strings.Contains(err.Error(), "404") {
		t.Logf("Primary URL failed with 404, trying fallback URL: %s", configFileUrlFallback)
		cmd[4] = configFileUrlFallback // Replace URL argument
		unsignedTx, err = testlib.ExecBin(t, ctx, xion.GetNode(), cmd...)
	}
	require.NoError(t, err)
	...
	err = testlib.UploadFileToContainer(t, ctx, xion.GetNode(), unsignedTxFile2)
	require.NoError(t, err)
