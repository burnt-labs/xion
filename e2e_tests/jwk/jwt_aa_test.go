...
func TestJWKJWTAA(t *testing.T) {
	...
	newAudienceClaimHash, err := testlib.ExecTx(t, ctx, xion.GetNode(), xionUser.KeyName(), "jwk", "create-audience-claim", newAud, "--chain-id", xion.Config().ChainID)
	require.NoError(t, err)
	t.Logf("new audience claim hash: %s", newAudienceClaimHash)
	txDetails, err = testlib.ExecQuery(t, ctx, xion.GetNode(), "tx", newAudienceClaimHash)
	require.NoError(t, err)
	t.Logf("TxDetails: %s", txDetails)
	newUpdateAudienceHash, err := testlib.ExecTx(t, ctx, xion.GetNode(),
		xionUser.KeyName(),
		"jwk", "update-audience",
		aud,
		string(testPublicKeyJSON),
	...
	audienceQuery, err := testlib.ExecQuery(t, ctx, xion.GetNode(), "jwk", "show-audience", newAud)
	require.NoError(t, err)
