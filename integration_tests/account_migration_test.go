...
func TestAbstractAccountMigration(t *testing.T) {
	...
	CosmosChainUpgradeIBCTest(t, &td, xionUser, "xion", "current", "xion", "upgrade", "v5")
	...
	fmt.Println(updatedCodeID)
	require.Equal(t, updatedCodeID, newCodeIDStr)
}

func registerAccounts(t *testing.T, noOfAccounts int, xionUser ibc.Wallet, ctx context.Context, tn *cosmos.ChainNode, keyName, codeIDStr string) {
	codeResp, err := ExecQuery(t, ctx, tn,
		"wasm", "code-info", codeIDStr)
	require.NoError(t, err)
	...
	for s := 0; s < noOfAccounts; s++ {
		salt := fmt.Sprintf("%d", s)
		creatorAddr := types.AccAddress(xionUser.Address())
		codeHash, err := hex.DecodeString(codeResp["data_hash"].(string))
		require.NoError(t, err)
		predictedAddr := wasmkeeper.BuildContractAddressPredictable(codeHash, creatorAddr, []byte(salt), []byte{})
		...
		txHash, err := ExecTxNoWait(t, ctx, tn, keyName, registerCmd...)
		require.NoError(t, err)
		t.Logf("tx hash: %s", txHash)
	}
}
