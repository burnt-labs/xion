...
func setupChainForAuth(t *testing.T) (TestData, ibc.Wallet, []byte, string, error) {
	...
	codeIDStr, err := xion.StoreContract(ctx, deployerAddr.FormattedAddress(),
		path.Join(fp, "testdata", "contracts", "simple_contract-aarch64.wasm"))
	require.NoError(t, err)
...
func TestXionCodeIdAuth(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("skipping in short mode")
	}

	td, deployerAddr, _, codeIDStr, err := setupChainForAuth(t)
	require.NoError(t, err)
	...
	txHash, err := ExecTx(t, ctx, xion.FullNodes[0], deployerAddr.KeyName(), instantiateCmd...)
	require.NoError(t, err)
	...
	sendMsg := fmt.Sprintf(`
	{
		...
		`, deployerAddr.FormattedAddress(), account.Address)
