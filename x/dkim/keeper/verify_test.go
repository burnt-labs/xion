...
func TestVerify(t *testing.T) {
	...
	require.NoError(t, err, "error decoding vkey", err.Error())
	t.Log(err)

	snarkVk, err := parser.UnmarshalCircomVerificationKeyJSON(vkey64Bz)
	require.NoError(t, err, "failed to unmarshal verification key: %v", err.Error())
	...
	require.NoError(t, err, "error decoding b64 %v", err.Error())
	...
	require.NoError(t, err, "failed to convert Circom proof to Gnark proof: %v", err.Error())

	verified, err := parser.VerifyProof(gnarkProof)
	require.NoError(t, err, "failed to verify proof :%v", err.Error())
...
func TestCalculateTxBodyCommitment(t *testing.T) {
	...
	assert.Equal(t, received, "14090993295699715988323746151124968754442881127975012622569574997457449326200")
}
