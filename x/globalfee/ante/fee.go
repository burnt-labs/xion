...
func (mfd FeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if mfd.ContainsOnlyBypassMinFeeMsgs(ctx, feeTx.GetMsgs()) {
		...
		// Guard: only clear when min-gas-prices is actually non-zero to avoid
		// spurious context mutations for validators already running with empty
		// minimum-gas-prices (including test environments).
